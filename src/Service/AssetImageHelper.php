...
class AssetImageHelper implements ContainerInjectionInterface {
  ...
  protected function setFallbackThumbnail($uri) {
    ...
    $source = $this->getGrediDamModulePath() . '/images/widen.png';
  ...
  /**
   * Get the path to the Gredi DAM module.
   *
   * This call is broken out for better flexibility when writing tests.
   *
   * @return string
  ...
  protected function getGrediDamModulePath() {
    ...
    return drupal_get_path('module', 'helfi_gredi_image');
  }

  /**
   * Saves the fallback thumbnail URI to the Gredi DAM config.
   *
   * This call is broken out for better flexibility when writing tests.
   *
   * @param string $uri
  ...
  protected function saveFallbackThumbnail($uri) {
    $this->configFactory->getEditable('helfi_gredi_image.settings')->set(
      'fallback_thumbnail',
      $uri
    )->save();
  }
